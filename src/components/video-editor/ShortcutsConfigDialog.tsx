...
export function ShortcutsConfigDialog() {
	...
	return (
		...
		>
			<DialogContent className="bg-[#17110f] border-white/10 text-white max-w-[420px]">
				<DialogHeader>
					<DialogTitle className="flex items-center gap-2 text-sm">
						<Keyboard className="w-4 h-4 text-[#FF6738]" />
						{t("title")}
					</DialogTitle>
				</DialogHeader>
								...
								<div className="flex items-center justify-between py-1.5 px-1 border-b border-white/5">
									...
									<button
										...
										className={[
											...
											isCapturing
												? "bg-[#FF6738]/20 border-[#FF6738] text-[#FF6738] animate-pulse"
												: hasConflict
													? "bg-amber-500/10 border-amber-500/50 text-amber-400"
													: "bg-white/5 border-white/10 text-slate-200 hover:border-[#FF6738]/50 hover:text-[#FF6738] cursor-pointer",
										].join(" ")}
									>
				...
				<DialogFooter className="flex gap-2 sm:justify-between mt-2">
					...
					<div className="flex gap-2">
						...
						<Button
							size="sm"
							className="bg-[#FF6738] hover:bg-[#E85A2F] text-white"
							onClick={handleSave}
						>
							{tc("actions.save")}
						</Button>
